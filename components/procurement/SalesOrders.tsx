...
import {
  FileText, Plus, X, Save, Loader, Search, Trash2,
  ChevronDown, ChevronUp, CheckCircle2, Clock, Truck,
  XCircle, User, Calendar, Receipt, Package
} from 'lucide-react';

interface SOLine { id?: string; item_id: string; quantity: number; unit_price: number; quantity_delivered?: number; subtotal?: number; item_name?: string; }
interface SO {
  id: string; name: string; partner_id: string; order_date: string;
  commitment_date?: string; state: string; total_amount: number; notes?: string;
  warehouse_id?: string; partner_name?: string; lines?: SOLine[];
}
interface Partner { id: string; name: string; partner_type: string; }
interface Item { id: string; name: string; sku?: string; sale_price?: number; }
...
const STATE_CFG: Record<string,{label:string;color:string;icon:React.ElementType}> = {
  ...
  shipped:   {label:'Shipped',   color:'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300',   icon:Truck},
  invoiced:  {label:'Invoiced',  color:'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300', icon:Receipt},
  cancelled: {label:'Cancelled', color:'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300',       icon:XCircle},
};
const FILTERS = ['all','draft','confirmed','shipped','invoiced','cancelled'] as const;
const EMPTY_FORM = {partner_id:'',commitment_date:'',warehouse_id:'',notes:''};
...
export const SalesOrders: React.FC = () => {
  ...
  const [lines, setLines]           = useState<SOLine[]>([{item_id:'',quantity:1,unit_price:0}]);
  const [saving, setSaving]         = useState(false);
  const [err, setErr]               = useState('');
  const [expanded, setExpanded]     = useState<string|null>(null);
  const [linesCache, setLinesCache] = useState<Record<string,SOLine[]>>({});
  ...
  const fetchMasters = useCallback(async () => {
    ...
    setPartners((p||[]).filter((x:any)=>x.partner_type==='customer'||x.partner_type==='both'));
    setItems((i||[]).map((x:any)=>({...x, sku: x.code, sale_price: 0})));
    setWarehouses(w||[]);
  }, []);
  ...
  const updateLine = (i:number, field:keyof SOLine, val:any) => {
    setLines(l=>l.map((ln,idx)=>{
      if(idx!==i) return ln;
      const updated={...ln,[field]:val};
      if(field==='item_id'){
        const item=items.find(it=>it.id===val);
        if(item?.sale_price) updated.unit_price=item.sale_price;
      }
      return updated;
    }));
  };
  ...
  const handleCreate = async () => {
    if (!form.partner_id){ setErr('Select a customer.'); return; }
    const valid=lines.filter(l=>l.item_id&&l.quantity>0);
    if (!valid.length){ setErr('Add at least one line item.'); return; }
    setSaving(true); setErr('');
    const {data,error} = await (supabase as any).rpc('rpc_create_sales_order',{
      p_partner_id: form.partner_id,
      p_commitment_date: form.commitment_date||null,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search SOs..."
          ...
          <button onClick={()=>{setForm(EMPTY_FORM);setLines([{item_id:'',quantity:1,unit_price:0}]);setErr('');setShowModal(true);}}
            className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700 transition-colors whitespace-nowrap">
      ...
      <div className="flex gap-2 flex-wrap">
        {FILTERS.map(f=>(
          <button key={f} onClick={()=>setFilter(f)}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${filter===f?'bg-emerald-600 text-white':'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>
      ...
      {/* List */}
      {loading ? (
        <div className="flex justify-center py-20"><Loader className="w-6 h-6 animate-spin text-emerald-500"/></div>
      ) : filtered.length===0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-600">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">No sales orders found</p>
          <button onClick={()=>{setForm(EMPTY_FORM);setLines([{item_id:'',quantity:1,unit_price:0}]);setErr('');setShowModal(true);}}
            className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700">Create SO</button>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map(so=>{
            const SC=STATE_CFG[so.state]||STATE_CFG.draft;
            const Icon=SC.icon;
            const cachedLines=linesCache[so.id]||[];
            return (
              <div key={so.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                <div className="p-4 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="font-bold text-slate-800 dark:text-white text-sm">{so.name}</span>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 flex items-center gap-1">
                      <User className="w-3 h-3 flex-shrink-0"/>{so.partner_name||'—'}
                      {so.commitment_date&&<><span className="text-slate-300 dark:text-slate-600 mx-1">·</span><Calendar className="w-3 h-3 flex-shrink-0"/>Delivery: {so.commitment_date}</>}
                    </p>
                    <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 mt-1">
                      QAR {Number(so.total_amount).toLocaleString('en',{minimumFractionDigits:2})}
                    </p>
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {so.state==='draft'&&(
                      <button onClick={()=>doAction(so.id,'rpc_confirm_sales_order')} disabled={!!actionLoading}
                    ...
                    {so.state==='confirmed'&&(
                      <button onClick={()=>doAction(so.id,'rpc_ship_sales_order')} disabled={!!actionLoading}
                        className="px-2.5 py-1 text-xs font-semibold bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors disabled:opacity-50 flex items-center gap-1">
                        <Truck className="w-3 h-3"/>Ship
                      </button>
                    )}
                    {so.state==='shipped'&&(
                      <button onClick={()=>doAction(so.id,'rpc_invoice_sales_order')} disabled={!!actionLoading}
                        className="px-2.5 py-1 text-xs font-semibold bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50 flex items-center gap-1">
                        <Receipt className="w-3 h-3"/>Invoice
                      </button>
                    )}
                    {['draft','confirmed'].includes(so.state)&&(
                      <button onClick={()=>doAction(so.id,'rpc_cancel_sales_order')} disabled={!!actionLoading}
                    ...
                    <button onClick={()=>toggleExpand(so.id)} className="p-1.5 text-slate-400 hover:text-slate-600 rounded-lg">
                      {expanded===so.id?<ChevronUp className="w-4 h-4"/>:<ChevronDown className="w-4 h-4"/>}
                    </button>
                  </div>
                </div>
                {expanded===so.id&&(
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>{cachedLines.map((l,i)=>(
                          <tr key={i} className="text-slate-600 dark:text-slate-300 border-b border-slate-50 dark:border-slate-700/30">
                            ...
                            <td className={`py-1 text-right font-semibold ${Number(l.quantity_delivered)>=Number(l.quantity)?'text-green-500':'text-amber-500'}`}>
                              {l.quantity_delivered||0}/{l.quantity}
                            </td>
                          </tr>
                        ))}</tbody>
                      </table>
                    )}
                    {so.notes&&<p className="text-xs text-slate-400 mt-2 italic">Notes: {so.notes}</p>}
                  </div>
                )}
              </div>
            );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
              <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><FileText className="w-5 h-5 text-emerald-500"/>New Sales Order</h3>
            ...
            <div className="p-5 overflow-y-auto flex-1 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <select value={form.partner_id} onChange={e=>setForm(f=>({...f,partner_id:e.target.value}))}
                    className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                    <option value="">Select customer...</option>
                    {customers.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Delivery Date</label>
                  <input type="date" value={form.commitment_date} onChange={e=>setForm(f=>({...f,commitment_date:e.target.value}))}
                    className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"/>
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} rows={2} placeholder="Optional notes..."
                    className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"/>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={addLine} className="flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 font-semibold">
                  ...
                  {lines.map((line,i)=>(
                    <div key={i} className="flex gap-2 items-center">
                      <select value={line.item_id} onChange={e=>updateLine(i,'item_id',e.target.value)}
                        className="flex-1 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500">
                      ...
                      <input type="number" min="0.01" step="0.01" value={line.quantity} onChange={e=>updateLine(i,'quantity',Number(e.target.value))}
                        placeholder="Qty" className="w-20 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 text-center"/>
                      <input type="number" min="0" step="0.01" value={line.unit_price} onChange={e=>updateLine(i,'unit_price',Number(e.target.value))}
                        placeholder="Price" className="w-24 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 text-right"/>
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-slate-200 dark:border-slate-700 flex-shrink-0">
              ...
              <button onClick={handleCreate} disabled={saving}
                className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-60">
                {saving?<Loader className="w-4 h-4 animate-spin"/>:<Save className="w-4 h-4"/>} Create SO
              </button>
            </div>
          </div>
        </div>
